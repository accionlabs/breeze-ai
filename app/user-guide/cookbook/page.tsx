...
export default function Cookbook() {
  return (
    <section className="docs-section">
      ...
      <p>Use when: you don&apos;t have a local dev env, you&apos;re a Product Owner / analyst, or the repo lives somewhere you can drop into the UI but not clone locally.</p>
...
/breeze:onboard-repository <path-to-repo>   # parses + uploads`}</pre>
      <p>The <code>/breeze:onboard-repository</code> skill auto-detects <code>nvm</code>, <code>fnm</code>, and <code>volta</code> and switches to Node 22 inline (the parser only runs on Node 22), then runs Tree-Sitter across 10 languages and streams the ndjson directly to the backend. Pass <code>--capture-statements</code> so downstream functional-from-backend has full method bodies to analyze.</p>
...
/breeze:update-functional-graph              # write the resulting nodes`}</pre>
      ...
      <ol>
        ...
        <li>Open the <b>Functional Chat</b> (<code>/chat/functional/:id</code>) and ask it to generate the graph from the uploaded documents.</li>
      </ol>
      ...
      <p><code>UserJourney → Flow → Page</code> plus the atomic system <code>Atom → Molecule → Organism → Template</code>. The skills here are well-documented; read the <code>SKILL.md</code> in each one for the full contract; the recipes are:</p>
      ...
      <p>Maps an incoming requirement to the eight-layer model, runs impact analysis via the code graph, anchors user-facing components to functional scenarios, and writes the analysis back (e.g. as a Jira comment). Use this for change-impact, not first-pass population.</p>
...
"Re-run architecture analysis for JIRA-1234; the design has been revised"`}</pre>
      ...
      <p>In a brownfield project the code already exists, so it is the only unambiguous source of truth available. The Functional, Design, and Architecture ontologies are <em>inferred from it</em>: this order is a practical extraction sequence, not a statement that Code is methodologically primary. (In greenfield work, Functional comes first; see below.)</p>
      <pre className="doc-pre">{`Code Graph   →   Functional Graph   →   Design Graph   →   Architecture Graph
(facts)          (intent)                (UI)               (system blueprint)`}</pre>
      <p>Each subsequent graph reads from the one before. The functional skills query the code graph for routes, components, and consumers, so code must exist first. The design skills traverse functional scenarios, so functional must come next. Architecture analysis draws on both functional and code for impact analysis.</p>
      ...
      <div className="doc-table-wrap">
        <table className="doc-table">
          ...
          <tbody>
            ...
            <tr><td>Functional from a requirement doc (UI)</td><td>WebUI → Knowledge Management → upload → Functional Chat</td></tr>
            ...
            <tr><td>Generate code and tests from the four-layer impact report</td><td><code>/breeze:generate-code</code></td></tr>
